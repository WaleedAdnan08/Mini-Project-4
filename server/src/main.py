...
def test():
    ...
    carp_park_positions_path = os.path.join(cwd, "server","data", "source", "Sample_3")
    video_path = os.path.join(cwd, "server","data", "source", "Sample_3.mp4")

    print(carp_park_positions_path)
    posList = load_parking_positions(carp_park_positions_path)
    if not posList:
        print("No parking positions loaded. Exiting...")
        return
    ...
    def checkParkingSpace(imgPro):
        ...
        cv2.putText(img, f"Free: {spaceCounter}/{len(posList)}", (10, 50), cv2.FONT_HERSHEY_PLAIN, 2, (0, 200, 0), 3)
